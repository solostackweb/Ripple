...
function SectionView({ name, onBack, openComposer }: { name: string; onBack: () => void; openComposer: () => void }) {
  ...
  return <section className="section-view"><button className="back-link" onClick={onBack}>← Overview</button><div className="section-title-row"><div><span className="eyebrow">Ripple workspace</span><h1>{title}</h1><p>{subtitle}</p></div>{name === "Changes" && <button className="primary-button" onClick={openComposer}><Icon name="plus" size={17}/>Report a change</button>}</div><SectionContent name={name}/></section>;
}
